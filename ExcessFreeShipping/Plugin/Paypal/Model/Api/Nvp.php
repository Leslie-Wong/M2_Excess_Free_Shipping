...
namespace Arlesfishes\ExcessFreeShipping\Plugin\Paypal\Model\Api;
...
class Nvp extends \Magento\Paypal\Model\Api\AbstractApi
{
    ...
    public function callSetExpressCheckout()
    {
        ...
        $getExcessFreeShippingValue = $objectManager->get('Magento\Quote\Model\Quote')
                    ->getCollection()
                    ->addFieldToFilter('reserved_order_id', array('eq' => $this->getInvNum()))
                    ->setOrder('updated_at','DESC')
                    ->getFirstItem()
                    ->getExcessFreeShippingValue();

        $logger->info('Plugin callSetExpressCheckout quote =>'. var_export(  $getExcessFreeShippingValue,true));
        
        if(floatval($getExcessFreeShippingValue) > 0){
            $request['ITEMAMT'] = $request['AMT'] - $request['TAXAMT'] - $getExcessFreeShippingValue;
        }
        // $request['ITEMAMT'] = $request['AMT'] - $request['TAXAMT'] - $request['SHIPPINGAMT'];
        // $request['SHIPPINGAMT'] = 0;
        
        // $logger->info('Shipping request 2 =>'. var_export($request,true));
    ...
    public function callDoExpressCheckoutPayment()
    {
        ...
        $logger->info('Plugin callDoExpressCheckoutPayment quote =>'. var_export(  $getExcessFreeShippingValue,true));
        
        if(floatval($getExcessFreeShippingValue) > 0){
            $request['AMT'] = $request['AMT'] + $getExcessFreeShippingValue;
        }
